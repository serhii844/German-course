  ...
  render() {
    ...
    if (["table", "five", "ten", "last10"].includes(this.mode)) {
      ...
      data.forEach((p, i) => {
        ...
        const skipBtn = `<button class="icon-btn" onclick="PairsAdjectives.skipRow(${i})">➡</button>`;

        row.innerHTML = `
          <td>${p.ua1} — ${p.ua2} ${skipBtn}</td>
          <td>
    ...
    if (this.mode === "single") {
      ...
      div.innerHTML = `
        ...
        <div class="single-controls">
          <button onclick="PairsAdjectives.checkSingle()">Перевірити</button>
          <button onclick="PairsAdjectives.switchMode('single')">Пропустити</button>
        </div>
        <p id="result"></p>
        <button id="nextBtn" style="display:none;" onclick="PairsAdjectives.render()">Наступна пара</button>
      `;
      app.appendChild(div);
    }
  },
